...
const questions = [
  "Be honest… you smiled at least once while reading this, didn’t you?",
  "Do you still pretend you don’t care?",
  "If I randomly texted you tomorrow, would you reply fast or act mysterious first?",
  "Are we actually fixing things right now?",
  "Do I still annoy you… or entertain you?",
  "Would you replay this website later when nobody’s watching?",
  "Did this website make me look slightly less dumb?",
];

const answerButtons = ["Maybe", "Depends", "Not telling you", "You wish", "A little", "Still deciding", "Shut up 😭"];

const playfulReplies = {
  Maybe: ["that’s basically a yes", "interesting response detected"],
  Depends: ["scientists are studying this answer", "conditional chemistry. noted."],
  "Not telling you": ["mysterious behavior logged", "that silence is suspiciously loud"],
  "You wish": ["okay that one hurt a little", "deflection detected. cute."],
  "A little": ["a little is a dangerous amount", "tiny smile confirmed by peer review"],
  "Still deciding": ["the jury is taking its sweet time", "fine. suspense looks good on you."],
  "Shut up 😭": ["that reaction has excellent chemistry", "laughing objection accepted"],
};
...
const analyticsConfig = window.NYASA_SUPABASE || {};
const sessionId = (() => {
  const key = "nyasa_session_id";
...
function getAnswersForQuestion(index) {
  return Array.from({ length: 4 }, (_, offset) => answerButtons[(index + offset * 2) % answerButtons.length]);
}
...
function storeLocalEvent(event) {
  try {
    const key = "nyasa_pending_interactions";
    const existing = JSON.parse(localStorage.getItem(key) || "[]");
    localStorage.setItem(key, JSON.stringify([...existing.slice(-80), event]));
  } catch (error) {
...
async function saveInteraction(event) {
  const payload = {
    ...
    event_type: event.eventType || "answer_click",
    question_text: event.questionText || null,
    selected_answer: event.selectedAnswer || null,
    page_path: window.location.pathname,
    device_info: getDeviceInfo(),
    section_dwell: event.sectionDwell || null,
    time_on_site_ms: event.timeOnSiteMs || null,
    created_at: new Date().toISOString(),
  };

  storeLocalEvent(payload);

  if (!analyticsConfig.url || !analyticsConfig.anonKey) {
    return { saved: false, offline: true };
  }

  const response = await fetch(`${analyticsConfig.url.replace(/\/$/, "")}/rest/v1/interactions`, {
    method: "POST",
    headers: {
      apikey: analyticsConfig.anonKey,
      Authorization: `Bearer ${analyticsConfig.anonKey}`,
      "Content-Type": "application/json",
      Prefer: "return=minimal",
    },
    body: JSON.stringify(payload),
    keepalive: event.keepalive || false,
  });

  if (!response.ok) throw new Error("Interaction save failed");
  return { saved: true };
}
...
function Hero() {
  return h("section", { id: "hero", className: "section" },
    ...
    h("div", { className: "section-inner" },
      ...
      h(motion.div, {
        className: "button-row hero-actions",
        initial: { y: 24, opacity: 0 },
        animate: { y: 0, opacity: 1 },
...
function QuestionCard({ question, index, onAnswer }) {
  ...
  const answers = useMemo(() => getAnswersForQuestion(index), [index]);
  ...
  const selectAnswer = async (answer, event) => {
    ripple(event);
    setSelected(answer);
    setSaving(true);
    const replies = playfulReplies[answer] || ["interesting response detected"];
    const line = replies[Math.floor(Math.random() * replies.length)];
    setReaction(line);
    onAnswer(answer, line, event);
    ...
    try {
      await saveInteraction({ questionText: question, selectedAnswer: answer });
    } catch (error) {
      setReaction(line);
    } finally {
      setSaving(false);
  ...
  return h(motion.article, {
    ref,
    className: cx("question-card glass-panel", selected && "answered", saving && "saving"),
    ...
    h("div", { className: "answer-bank" },
      answers.map((answer) => h("button", {
        key: answer,
        className: cx("mini-btn", selected === answer && "selected"),
        disabled: saving,
        onClick: (event) => selectAnswer(answer, event),
      }, saving && selected === answer ? "Wait..." : answer)),
    ),
    h(AnimatePresence, null,
      reaction && h(motion.p, {
        key: reaction,
        className: "question-reaction",
...
function Questions({ addReply }) {
  return h("section", { id: "questions", className: "section questions-section" },
    h("div", { className: "section-inner" },
      h("p", { className: "eyebrow reveal" }, "Chemistry check"),
      h("h2", { className: "text-4xl md:text-7xl font-extrabold leading-none max-w-4xl reveal" }, "A few questions, asked with calm confidence and very suspicious timing."),
      h("div", { className: "question-grid mt-12" },
        questions.map((question, index) => h(QuestionCard, {
          ...
          index,
          onAnswer: (answer, line, event) => {
            addReply(line, event.clientX, event.clientY, true);
          },
        })),
      ),
    ),
...
function FloatingReplies({ replies }) {
  return h(AnimatePresence, null,
    replies.map((reply) => h(motion.div, {
      ...
      reply.particles && Array.from({ length: 6 }, (_, index) => h(motion.span, {
        key: index,
        className: "response-particle",
        style: {
          "--burst-x": `${Math.cos((index / 6) * Math.PI * 2) * (28 + index * 3)}px`,
          "--burst-y": `${Math.sin((index / 6) * Math.PI * 2) * (24 + index * 2)}px`,
        },
        initial: { opacity: 0, x: 0, y: 0, scale: 0.35 },
        animate: { opacity: [0, 1, 0], x: `var(--burst-x)`, y: `var(--burst-y)`, scale: [0.35, 1, 0.65] },
...
function useSiteAnalytics() {
  ...
  useEffect(() => {
    const commit = () => {
      ...
      if (!id) return;
      dwell.current[id] = (dwell.current[id] || 0) + Date.now() - activeSince.current;
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      if (!visible?.target?.id || visible.target.id === activeSection.current) return;
      commit();
    ...
    const flush = () => {
      ...
      saveInteraction({
        ...
        sectionDwell: dwell.current,
        timeOnSiteMs: Date.now() - startedAt.current,
        keepalive: true,
      }).catch(() => {});
    };
...
function App() {
  ...
  const addReply = (text, x, y, particles = false) => {
    const id = `${Date.now()}-${Math.random()}`;
    setReplies((items) => [...items, { id, text, x, y, particles }]);
